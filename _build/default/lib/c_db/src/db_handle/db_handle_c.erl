...
-export([key1st/4, index1st/5, lastKey/4, lastIndex/5]).
...
-export([checkIterateIndex/3, iterate/1, iterate_end/1]).
-export([clear/3]).
-export([is_tab/2]).
...
get_all_k(Src, Table, TimeOut) when is_integer(TimeOut) andalso TimeOut > 0 ->
    is_tab(Src, Table),
    #{state := ok, data := Value} = gen_server:call('db_handle_proc', #{tab_keys => {Src, Table},
        type => 'get_all_k', 'time_out' => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    Value.
...
get_values(Src, Table, Num, TimeOut) when is_integer(TimeOut) andalso TimeOut > 0 ->
    is_tab(Src, Table),
    #{state := ok, data := Value} = gen_server:call('db_handle_proc', #{tab_keys => {{Src, Table}, Num},
        type => 'get_values', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    Value.
...
member(Src, Table, Key, TimeOut) when is_integer(TimeOut) andalso TimeOut > 0 ->
    is_tab(Src, Table),
    #{state := ok, data := Value} = gen_server:call('db_handle_proc', #{tab_keys => {{Src, Table}, Key},
        type => 'member', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    Value.
...
get_count(Src, Table, TimeOut) when is_integer(TimeOut) andalso TimeOut > 0 ->
    is_tab(Src, Table),
    #{state := ok, data := Value} = gen_server:call('db_handle_proc', #{tab_keys => {Src, Table},
        type => 'table_count', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    Value.
...
get(Src, Table, Key, Default, TimeOut) when is_integer(TimeOut) andalso TimeOut > 0 ->
    is_tab(Src, Table),
    #{state := ok, data := [{_, Value}]} = gen_server:call('db_handle_proc', #{tab_keys => [{{Src, Table}, Key}],
        type => 'get', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    case Value of
        none -> Default;
        Value -> Value
    end.
...
key1st(Src, Table, Default, TimeOut) ->
    is_tab(Src, Table),
    #{state := ok, data := Value} = gen_server:call('db_handle_proc', #{tab_keys => {Src, Table},
        type => 'key1st', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    case Value of
        none -> Default;
        Value -> Value
    end.
...
index1st(Src, Table, Pos, Default, TimeOut) ->
    is_tab(Src, Table),
    checkPos(Table, [{Pos, none}]),
    #{state := ok, data := Value} = gen_server:call('db_handle_proc', #{tab_keys => {{Src, Table}, Pos},
        type => 'index1st', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    case Value of
        none -> Default;
        Value -> Value
    end.
...
lastKey(Src, Table, Default, TimeOut) ->
    is_tab(Src, Table),
    #{state := ok, data := Value} = gen_server:call('db_handle_proc', #{tab_keys => {Src, Table},
        type => 'lastKey', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    case Value of
        none -> Default;
        Value -> Value
    end.
...
lastIndex(Src, Table, Pos, Default, TimeOut) ->
    ...
    checkPos(Table, [{Pos, none}]),
    #{state := ok, data := Value} = gen_server:call('db_handle_proc', #{tab_keys => {{Src, Table}, Pos},
        type => 'lastIndex', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    case Value of
        none -> Default;
        Value -> Value
    end.
...
%% @end
%%%===================================================================
getKey(Src, Table, PosIndexL, Default, TimeOut) when is_integer(TimeOut) andalso TimeOut > 0 ->
    is_tab(Src, Table),
    checkPos(Table, PosIndexL),
    #{state := ok, data := DataL} = gen_server:call('db_handle_proc', #{tab_keys => [{{Src, Table}, PosIndexL}],
        type => 'getKey', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    [case Value of
        none -> Default;
        _ -> Value
    end || Value <- DataL].
...
m_get(Src, Table, Keys, Default, TimeOut) when is_integer(TimeOut) andalso TimeOut > 0 ->
    ...
    #{state := ok, data := DataL} = gen_server:call('db_handle_proc', #{tab_keys => TabKeys,
        type => 'get', time_out => TimeOut, 'chanFlag' => true}, TimeOut + ?FAULT_TOLERANT_TIME_M_SEC),
    [case Value of
        none -> Default;
        Value -> Value
...
%% @end
%%%===================================================================
checkIterateIndex(Src, Table, Index) ->
    is_tab(Src, Table),
    checkPos(Table, [{Index, none}]).
...
checkPos(Table, PosIndexL) ->
    T = dbT:get_mod(),
    {_, Mod} = o_tuple:get(Table, T),
    case is_atom(Mod) of
        true ->
            case erlang:function_exported(Mod, indexPos, 0) of
                true ->
                    IndexPos = Mod:indexPos(),
                    IndexPosL = case is_list(IndexPos) of true -> IndexPos; false -> [IndexPos] end,
                    F = fun(Pos) ->
                        [error({Pos, IndexPosL, "index table pos error"}) || not lists:member(Pos, IndexPosL)]
                    end,
                    [F(Pos) || {Pos, _} <- PosIndexL];
                false -> error({Table, "index table no indexPos funcation"})
            end;
        false -> error({Table, "index table no mod"})
    end.
