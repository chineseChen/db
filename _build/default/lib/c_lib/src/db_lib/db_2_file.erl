...
write_2_file(ModStr, Cfg, EFd, HFd) ->
    ...
    IndexPos = indexPos(ColumnL, InfoL),
    ...
    ExFuncStr = case IndexPos of
        none -> [InitEStr, DBSrcEStr, KeyInEStr, CheckEStr];
        _ -> [InitEStr, DBSrcEStr, KeyInEStr, element(2, IndexPos), CheckEStr]
    end,
    io:put_chars(EFd, export_func(ExFuncStr)),
    ...
    FF = fun() ->
        ...
        ?BOOL_VALUE(IndexPos =:= none, ok, io:put_chars(EFd, element(1, IndexPos))),
...
func_init_str(TagStr, [_ | _] = KeyColumn) ->
    ...
    F = fun(Column, {Index, AAcc, BAcc, CAcc}) ->
        ...
        case TypeStr1 =:= "term" of
            true ->
                KeyVar = "Key" ++ integer_to_list(Index),
                {Index - 1, [KeyVar | AAcc], BAcc, [RecKey ++ " = " ++ KeyVar | CAcc]};
            false ->
                KeyVar = "Key" ++ integer_to_list(Index),
...
func_index(Index, Var, EType, SType, Comment) ->
    ...
    Str = io_lib:format("{<<\"~s\">>, '~s', ~s, <<\"~ts\"/utf8>>}", [Var, EType, str_lib:term_2_str(SType), unicode:characters_to_binary(Comment)]),
...
key_index(ColumnL, InfoL) ->
    case proplists:get_value('primary_key', InfoL, none) of
        none -> {?RECORD_1ST_FIELD_POS, element(1, hd(ColumnL))};
        [_, _ | _] = ColumnKey -> keyIndex_l(ColumnL, ColumnKey);
        [ColumnKey] -> keyIndex_1(ColumnL, ColumnKey);
        ColumnKey -> keyIndex_1(ColumnL, ColumnKey)
    end.
...
%% @end
%%%===================================================================
indexPos(ColumnL, InfoL) ->
    F = fun(Info) ->
        case Info of
            {Key, Value} when Key =:= 'key' -> {true, Value};
            _ -> false
        end
    end,
    IndexTagL = lists:filtermap(F, InfoL),
    true = length(lists:usort(IndexTagL)) =:= length(IndexTagL),
    case IndexTagL of
        [] -> none;
        [_, _ | _] = IndexTagL -> indexPos_l(ColumnL, IndexTagL);
        [IndexTag] -> indexPos_1(ColumnL, IndexTag)
    end.
...
%% @end
%%%===================================================================
indexPos_1(ColumnL, IndexTag) ->
    F = fun(Column, Index) ->
        case element(1, Column) of
            IndexTag -> {break, {ok, Index}};
            _ -> Index + 1
        end
    end,
    {ok, Index} = c_lib:foreach(F, ?RECORD_1ST_FIELD_POS, ColumnL),
...
indexPos_l(ColumnL, IndexTagL) ->
    F = fun(Key, Acc) ->
        ...
        {ok, Index} = c_lib:foreach(FF, ?RECORD_1ST_FIELD_POS, ColumnL),
        [Index | Acc]
    end,
    IndexL = lists:reverse(c_lib:foreach(F, [], IndexTagL)),
    {"indexPos() ->\r\n\t" ++ str_lib:term_2_str(IndexL) ++ ".\r\n\r\n", "indexPos/0"}.
...
keyIndex_l(ColumnL, ColumnKey) ->
    F = fun(Key, Acc) ->
        ...
        {ok, Index, Column} = c_lib:foreach(FF, ?RECORD_1ST_FIELD_POS, ColumnL),
        [{Index, Column} | Acc]
    end,
...
keyIndex_1(ColumnL, ColumnKey) ->
    ...
    {ok, Index, Column} = c_lib:foreach(F, ?RECORD_1ST_FIELD_POS, ColumnL),
    {"key_index() ->\r\n\t" ++ integer_to_list(Index) ++ ".\r\n\r\n", "key_index/0", Column}.
