...
-module(biT_test).
-author(sql).
-include("biT_test.hrl").

-export([init/0, init/1, db_src/0, key_index/0, check/1]).
-export([get_id/1, get_key1/1, get_var1/1, get_var2/1, get_var3/1, get_var4/1, get_var5/1, get_var6/1, get_var7/1, get_var8/1]).
-export([set_id/2, set_key1/2, set_var1/2, set_var2/2, set_var3/2, set_var4/2, set_var5/2, set_var6/2, set_var7/2, set_var8/2]).
-export([save_type/0]).
...
set_id(R, Var) when is_integer(Var) ->
	R#test{id = Var}.
...
select(DB) ->
	list_to_binary(io_lib:format(<<"SELECT * FROM ~s.`test` WHERE `id` = ?;">>, [DB])).

delete(DB) ->
	list_to_binary(io_lib:format(<<"DELETE FROM ~s.`test` WHERE `id` = ?;">>, [DB])).
