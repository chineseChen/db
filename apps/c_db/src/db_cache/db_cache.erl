...
new_tab(none, Arg) ->
    ...
    Arg#cacheMgr{diskBool = false, etsCache = CatchTRef, etsCacheInfo = CatchInfoRef, etsLockQueue = LockTRef, etsLock = RunTRef};
new_tab(Mod, Arg) when is_atom(Mod) ->
    ...
    put(?DB_ETS_TAB_LOCK_QUEUE, LockTRef),
    put(?DB_ETS_TAB_LOCK_RUN, RunTRef),
    Arg0 = Arg#cacheMgr{etsCacheInfo = CatchInfoRef, etsLockQueue = LockTRef, etsLock = RunTRef},
...
init_key() ->
    [begin
        ...
        HStr = io_lib:format("select ~s from ~s.~s", [PriKey, DBName, TableName]),
        QueryStr1 = unicode:characters_to_binary(io_lib:format("~s order by ~s ASC limit ~w;", [HStr, PriKey, ?CACHE_INIT_FROM_SQL_PAGE_NUM])),
        PriKeyL1 = initKeyByQuery(KeyTRef, ErlType, QueryStr1),
        F = fun(LastPriKeyL) ->
            case LastPriKeyL of
                [_ | _] ->
                    [MaxVar0] = lists:last(LastPriKeyL),
                    QueryStr2 = unicode:characters_to_binary(io_lib:format("~s WHERE ~s > ~p order by ~s ASC limit ~w;",
                        [HStr, PriKey, formatWhereConVar(ErlType, MaxVar0), PriKey, ?CACHE_INIT_FROM_SQL_PAGE_NUM])),
                    initKeyByQuery(KeyTRef, ErlType, QueryStr2);
                _ -> {break, ok}
            end
        end,
        c_lib:while(F, PriKeyL1)
...
%% @end
%%%===================================================================
initKeyByQuery(KeyTRef, ErlType, QueryStr) ->
    SqlPid = db_sql:getReadPid(),
    PriKeyL = case catch mysql:query(SqlPid, QueryStr, 60000) of
        {ok, _, PriKeyL0} ->
            PriKeyL0;
        MysqlError ->
            error({MysqlError, QueryStr})
    end,
    KeyF = fun(PriKeyVar) -> {db_sql_data:erl_value_by_type(PriKeyVar, ErlType)} end,
    ets:insert(KeyTRef, [KeyF(PriKeyVar) || [PriKeyVar] <- PriKeyL]),
    PriKeyL.
...
clearDiskData(Src) ->
    ...
    [begin
        ...
        DBName = gen_server:call('db_sql_mgr', {'get_db_name', Src}),
...
rollBack(Running, CatchInfoRef, CatchTRef, Key) ->
    [doRollBack(Running#running.data, CatchInfoRef, CatchTRef, Key)
        || not is_process_alive(Running#running.pid)].
